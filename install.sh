...
main() {
    ...
    command -v jq >/dev/null 2>&1 || { echo >&2 "jq is required but not installed. Installing jq..."; curl -s "https://github.com/stedolan/jq/releases/download/jq-1.6/jq-osx64" -o ./jq && chmod +x ./jq; }
    ...
    if [ ! -f "./RobloxPlayer.zip" ]; then
        echo "Failed to download RobloxPlayer.zip"
        exit 1
    fi
    ...
    # Check if the app directory was created after unzip
    if [ ! -d "./RobloxPlayer.app" ]; then
        echo "Failed to unzip RobloxPlayer.zip"
        exit 1
    fi
